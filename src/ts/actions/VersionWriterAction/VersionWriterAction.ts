...
export class VersionWriteAction extends Action {
     ...
     * files, key, and version string or a flag to retrieve it from `package.json`.
     * @returns A Promise that resolves when the version has been successfully
     * replaced in all specified files, or rejects with an error if the action fails.
     */
        ...
        try {
            ...
            await Promise.all(
                files.map(({ path: filePath, key }) =>
                    this.replaceVersionInFile(filePath, version!, key || "version:")
                )
            );
     ...
     * This method reads the file, replaces the version for the key, and writes the updated
     ...
     * @param key - The key prefix that identifies the version line (e.g., `version:`).
     * @returns A promise that resolves when the version has been successfully replaced.
    ...
    private async replaceVersionInFile(
        ...
        try {
            ...
            throw new Error(
                `Error replacing version in file "${filePath}": ${(error as Error).message}`
            );
        }
    }
     ...
     */
    describe(): string {
        return "Replaces a version string in one or more specified files based on a key and pattern. Can retrieve the version from package.json or set it manually.";
    }
}
