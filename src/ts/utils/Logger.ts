...
export class Logger {
     ...
     * @param style - The style to apply to the log level.
    ...
    private message(
        ...
        return `${fgStyle}${bgStyle}[${level}]${LoggerStyles.Reset} [${LoggerStyles.Cyan}${context}${LoggerStyles.Reset}] ${message}`;
    }
    ...
    public logError(
        ...
        if (error instanceof Error && this.verbose && error.stack) {
            console.error(`${LoggerStyles.Red}Stack trace:${LoggerStyles.Reset}\n${error.stack}`);
        }
